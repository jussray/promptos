#!/usr/bin/env python3
"""Validate that an approved PromptOS deployment candidate is still production-equivalent to main.
...
def main() -> None:
    ...
    changed = [line.strip() for line in git("diff", "--name-only", f"{candidate}..{current}").splitlines() if line.strip()]
